      ...
      <thead>
        <tr>
          ...
          <th>
            <el-tooltip placement="top" effect="light">
              <div slot="content">
                ...
                纵轴是质量得分，Q =-10*log10(p)，p 为测错的概率。所以一条 reads 某位置出错概率 0.01 时，其 quality 就是 20。横轴是测序序列的位置。蓝色线是各个位置的平均值的连线。一般要求此图中，所有位置的 10% 分位数大于 20，也就是常说的 Q20 过滤。<br>
          ...
          <th>
            <el-tooltip placement="top" effect="light">
              <div slot="content">
                - 这一模块是检查在测序平台上，reads 中每一个碱基位置在不同的测序小孔之间的偏离度，偏离度越高，碱基质量越差。纵轴表示测序小孔，蓝色表示低于平均偏离度，越红则说明偏离平均质量方差越多，也就是说质量越差，本图中都是蓝色表明质量很好。如果出现质量问题可能是短暂的，如有气泡产生，也可能是长期的，如在某一小孔中存在杂质。 <br>
                - 偏离度小于平均值2以上报警；<br>
                - 偏离度小于平均值5以上不合格。
              </div>
              <span>每瓦质量</span>
          ...
          <th>
            <el-tooltip placement="top" effect="light">
              <div slot="content">
                - 这是为了检测一部分质量特别差的 reads，如果有则会在图上出现多个峰，如在测序仪边缘的 reads。纵轴是 reads 数目，横轴是质量分数，代表不同 Phred 值对应了多少的 reads。 <br>
          ...
          <th>
            <el-tooltip placement="top" effect="light">
              <div slot="content">
                - 图中红色曲线是实际的测序 GC 含量分布图，而蓝色曲线则是理论分布（正态分布，不过均值不一定都是 50%，而是由平均 GC 含量推断的）。如果红色曲线形状存在比较大的偏差，往往是由于文库的污染造成的。红色曲线越平滑越好，越接近蓝色曲线越好。形状接近正态但偏离理论分布的情况提示我们可能有系统偏差。 <br>
          ...
          <th>
            <el-tooltip placement="top" effect="light">
              <div slot="content">
                - 纵轴是百分含量，横轴是 read 的位置，当测序仪不能确切地测定出某一个碱基时就会标注为 N，正常情况下 N 的比例是很小的，所以图上常常看到一条直线。当看到有峰时，说明测序出了问题。 <br>
          ...
          <th>
            <el-tooltip placement="top" effect="light">
              <div slot="content">
                - 每次测序仪测出来的长度在理论上应该是完全相等的，但是总会有一些偏差，当测序的长度有很大不同时，则表明测序仪在此次测序过程中产生的数据不可信，但对于某些测序平台，具有不同的 read 长度是完全正常的。 <br>
                - 当 reads 长度不一致时警告； <br>
                - 当有长度为 0 的 read 时不合格。
              </div>
              <span>序列长度分布</span>
        ...
        <tr v-for="(item, index) in tableData">
            ...
            <td :class="{'bgcolor': index % 2 === 0 ? false: true}">
              <el-tooltip placement="top" effect="light">
                <div slot="content">
                  <div class="tdImg">
                    <img class="" :src="'/projects/' + $store.state.projectId+ '/results/006.fastqc/' + item.fileName + '_fastqc/Images/' + 'per_base_quality.png'" alt="">
                  </div>
                </div>
            ...
            <td :class="{'bgcolor': index % 2 === 0 ? false: true}">
              <el-tooltip placement="top" effect="light">
                <div slot="content">
                  <div class="tdImg">
                    <img class="" :src="'/projects/' + $store.state.projectId+ '/results/006.fastqc/' + item.fileName + '_fastqc/Images/' + 'per_tile_quality.png'" alt="">
                  </div>
                </div>
            ...
            <td :class="{'bgcolor': index % 2 === 0 ? false: true}">
              <el-tooltip placement="top" effect="light">
                <div slot="content">
                  <div class="tdImg">
                    <img class="" :src="'/projects/' + $store.state.projectId+ '/results/006.fastqc/' + item.fileName + '_fastqc/Images/' + 'per_sequence_quality.png'" alt="">
                  </div>
                </div>
            ...
            <td :class="{'bgcolor': index % 2 === 0 ? false: true}">
              <el-tooltip placement="top" effect="light">
                <div slot="content">
                  <div class="tdImg">
                    <img class="" :src="'/projects/' + $store.state.projectId+ '/results/006.fastqc/' + item.fileName + '_fastqc/Images/' + 'per_sequence_gc_content.png'" alt="">
                  </div>
                </div>
            ...
            <td :class="{'bgcolor': index % 2 === 0 ? false: true}">
              <el-tooltip placement="top" effect="light">
                <div slot="content">
                  <div class="tdImg">
                    <img class="" :src="'/projects/' + $store.state.projectId+ '/results/006.fastqc/' + item.fileName + '_fastqc/Images/' + 'per_base_n_content.png'" alt="">
                  </div>
                </div>
            ...
            <td :class="{'bgcolor': index % 2 === 0 ? false: true}">
              <el-tooltip placement="top" effect="light">
                <div slot="content">
                  <div class="tdImg">
                    <img class="" :src="'/projects/' + $store.state.projectId+ '/results/006.fastqc/' + item.fileName + '_fastqc/Images/' + 'sequence_length_distribution.png'" alt="">
                  </div>
                </div>
            ...
            <td :class="{'bgcolor': index % 2 === 0 ? false: true}">
              <el-tooltip placement="top" effect="light">
                <div slot="content">
                  <div class="tdImg">
                    <img class="" :src="'/projects/' + $store.state.projectId+ '/results/006.fastqc/' + item.fileName + '_fastqc/Images/' + 'adapter_content.png'" alt="">
                  </div>
                </div>
