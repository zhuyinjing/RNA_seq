  ...
  <div class="">
    ...
    <div class="content">
      <el-breadcrumb separator="/" style="margin:5px 0 50px 0">
        ...
        <el-breadcrumb-item>富集分析</el-breadcrumb-item>
      </el-breadcrumb>

      <h2>富集分析 {{$store.state._case}} vs {{$store.state._control}} </h2>
  ...
  methods: {
    ...
    getStatus () {
      this.timer = setInterval(() => {
        this.axios.get('/server/enrich_do_status?username=' + this.$store.state.username + '&p=' + this.$store.state.projectId).then((res) => {
          if (res.data.message_type === 'success') {
            this.loading.close()
